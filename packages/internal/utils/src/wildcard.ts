...
/**
 * Compiles one or more sample strings into a matcher function.
 * The matcher takes a glob pattern (or array of patterns) and returns `true`
 * if any of the provided samples match the pattern(s).
 *
 * ```js
 * matchWildcard('test.example.com')('*.example.com') //=> true
 * ```
 *
 * ```js
 * const isMatch = matchWildcard(['foo.js', 'bar.ts'], '.')
 * isMatch('*.js') //=> true
 * isMatch('*.ts') //=> true
 * isMatch('*.json') //=> false
 * ```
...
export function matchWildcard(sample: string, options?: string | boolean | WildcardMatchOptions) {
	...
	const fn = ((pattern: string | string[]) => {
		if (typeof pattern !== "string" && !Array.isArray(pattern)) {
			throw new TypeError(
				`Pattern must be a string or array of strings, but ${typeof pattern} given`,
			);
		}
	...
	fn.options = options || {};
	fn.pattern = sample;
	fn.regexp = new RegExp(
		`^${escapeRegExpString(transform(sample, options?.separator))}$`,
		options?.flags,
	);
