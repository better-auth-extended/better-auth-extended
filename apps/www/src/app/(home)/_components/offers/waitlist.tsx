...
const lettersAnimationDuration = 0.1 * 1000 * totalLetters + 1500;

export const WaitlistOffer = () => {
	const { hovering: isHovering } = useBentoGridItem();
	...
	const emailPlaceholderDisplayTimeout = useRef<NodeJS.Timeout | null>(null);
	const lettersInterval = useRef<NodeJS.Timeout | null>(null);
	const loadingSpinner = useRef<NodeJS.Timeout | null>(null);
	const autoRestartLoopTimer = useRef<NodeJS.Timeout | null>(null);
	...
	return (
		<motion.div
			className={cn(
				"w-full h-full py-5 px-2 select-none transition-opacity duration-150 ease-in-out",
...
WaitlistOffer.displayName = "WaitlistOffer";
